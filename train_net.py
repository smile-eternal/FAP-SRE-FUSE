...
parser.add_argument('--train_path', type=str, default='./data/train_raw_blur_pair_2k.pkl', help='train image list')#train_pair_20k
parser.add_argument('--valid_path', type=str, default='./data/disp10.pkl', help='validation image list')
...
parser.add_argument('--lr_init', type=float, default=0.0001, help='learning rate for generator')
parser.add_argument('--max_epoch', type=int, default=500, help='number of epochs to train for')
...
for epoch in range(opt.max_epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        labels = torch.cat((gt1, gt2), 1).requires_grad_(False)
        optimizer.zero_grad()
        outputs = net(inputs)

        loss_pair_lable = criterion(outputs,labels)
        g1 = outputs[:,0,:,:].unsqueeze(1) * (input1.data )
        g2 = outputs[:, 1, :, :].unsqueeze(1) * (input2.data)
        mergeimg = g1 + g2
        loss_ssim = 1.0 - ssim_loss(mergeimg.cuda(),gtimg.cuda())
        loss_gradients = GL.gradient_loss_merge(mergeimg,gtimg,opt.cuda,device=0)
        sumpreds = outputs[:,0,:,:] + outputs[:,1,:,:]
